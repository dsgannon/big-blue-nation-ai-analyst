...
def get_opponent_player_stats(opp_team, game_id, game_date, opponent_name, season):
    """Parse opponent player stats for building opponent offensive rolling features"""
    if not opp_team:
        return []

    player_stats = []
    for stats_group in opp_team.get("statistics", []):
...
def get_game_boxscore(game_id, game_date, season="2025-26"):
    ...
    # Team-level opponent totals (for opponent_game_stats table)
    opp_stats = get_team_totals(opp_team) if opp_team else {}

    # Opponent player-level stats (for building rolling offensive features)
    opp_player_stats = get_opponent_player_stats(
        opp_team, game_id, game_date, opponent_name, season
    ) if opp_team else []

    return player_stats, opp_stats, opp_player_stats
...
def get_season_boxscores(schedule, season="2025-26"):
    ...
    for i, game in enumerate(completed):
        ...
        try:
            ...
            if result:
                player_stats, opp_stats, opp_player_stats = result
                all_player_stats.extend(player_stats)
                all_opp_player_stats.extend(opp_player_stats)

                opp_stats["game_id"] = game_id
                opp_stats["season"] = season
                opp_stats["opponent"] = (
                    game.get("away_team")
    ...
    return all_player_stats, all_opp_stats, all_opp_player_stats
...
if __name__ == "__main__":
    ...
    current_stats, current_opp, current_opp_players = get_season_boxscores(current_schedule, season="2025-26")
    print(f"  Player records: {len(current_stats)}")
    print(f"  Opponent player records: {len(current_opp_players)}")

    # Previous season
    print("\n📊 2024-25 Season (previous)...")
    prev_schedule = get_previous_season_schedule("2025")
    prev_stats, prev_opp, prev_opp_players = get_season_boxscores(prev_schedule, season="2024-25")
    print(f"  Player records: {len(prev_stats)}")
    print(f"  Opponent player records: {len(prev_opp_players)}")

    # Combine
    all_real_stats = current_stats + prev_stats
    all_opp_player_stats = current_opp_players + prev_opp_players
    print(f"\n✅ Total Kentucky records: {len(all_real_stats)}")
    print(f"✅ Total opponent player records: {len(all_opp_player_stats)}")
